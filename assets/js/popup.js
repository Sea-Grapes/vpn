...
var app = angular
    .module("app", ["frapontillo.bootstrap-switch", "ui.bootstrap"])
...
app.controller("HeaderCtrl", [
    ...
    function ($scope) {
        ...
        $scope.feedbackMessage =
            chrome.i18n.getMessage("headerFeedbackMessage") +
            " info89@ultrasurf.us";
...
app.controller("StateCtrl", [
    ...
    function ($scope) {
        ...
        var iid = setInterval(function () {
            ...
            $scope.statusMessage = $scope.state
                ? enableMessage
                : disableMessage;
            $scope.$apply();
        }, 200);
...
app.controller("StatusControl", [
    ...
    function ($scope) {
        ...
        $scope.troubleConnectingMessage = chrome.i18n.getMessage(
            "openDiagnosticsMessage"
        );
        ...
        $scope.feedbackRequestMessage = chrome.i18n.getMessage(
            "feedbackRequestMessage"
        );
        $scope.submitButtonText = chrome.i18n.getMessage("submitButtonText");
        $scope.thankYouForFeedbackMessage = chrome.i18n.getMessage(
            "thankYouForFeedbackMessage"
        );
        $scope.rateExtensionButtonText = chrome.i18n.getMessage(
            "rateExtensionButtonText"
        );
...
angular.module("app").controller("RatingCtrl", [
    "$scope",
    function ($scope) {
        $scope.isCollapsed = true;
        $scope.askForStoreRating =
            localStorage["ratingGiven"] === "5" &&
        ...
        $scope.openRatingsPage = function () {
            ...
            /*chrome.tabs.create({
                url:
                    "https://chrome.google.com/webstore/detail/ultrasurf/mjnbclmflcpookeapghfhapeffmpodij/reviews"
            });*/
        };
        var iid;
